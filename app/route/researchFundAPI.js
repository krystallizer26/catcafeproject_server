...
router.get('/researchFund/', function (request, response) {
    var methodCode = "XF";
    ...
    if (!requiredReady) {
        ...
        flow.exec(
            function () {
                ResearchFund_Control.getResearchFundByResearcherId(new ObjectId(request.query.researcherId), this);
            },
...
router.get('/admin/researchFund',async function (request, response) {
    var methodCode = "XF";
    ...
    if (!requiredReady) {
        ...
        if(executable) {
            flow.exec(
                function () {
                    ResearchFund_Control.getResearchFundByAdmin(researcherId, this);
                },
                function (code, err, getResponse) {
                    if (err) {
                        Return.responseWithCode(ReturnCode.serviceError + methodCode + code, err, response);
...
router.get('/admin/researchFund/:researchFundId',async function (request, response) {
    var methodCode = "XF";

    var requiredData = [];
    requiredData.push(request.params.researchFundId);
    requiredData.push(request.headers.token);
    var requiredReady = Validate.requiredData_Check(requiredData)

    console.log("Validation 3")
    var objectIdData = [];
    objectIdData.push(request.params.researchFundId);
    var objectIdReady = Validate.objectIDData_Check(objectIdData)
    ...
    if (!requiredReady) {
        ...
        if(user == "token is expired" || user == "user not found"){
            ...
            flow.exec(
                function () {
                    ResearchFund_Control.getResearchFundForResearcherIdByAdmin(new ObjectId(request.params.researchFundId), this);
                },
                function (code, err, getResponse) {
                    if (err) {
                        Return.responseWithCode(ReturnCode.serviceError + methodCode + code, err, response);
                    } else {
                        Return.responseWithCodeAndData(ReturnCode.success, "getResearchFundForResearcherIdByAdmin Successfully executed", getResponse, response);
                    }
                }
            );
        }
...
router.post('/admin/researchFund/', userController.checkUserInfo, async function (request, response) { //เพิ่ม dev. เป็น noti req.
    var methodCode = "XF";
    ...
    if (!requiredReady) {
        ...
        flow.exec(
            function () {
                ...
                let dataUpdate = {}
                dataUpdate.dataType = 'researchFund'
...
router.post('/admin/researchFund/:researcherId', userController.checkUserInfo, async function (request, response) { //แก้ไข dev. เป็น noti req.
    var methodCode = "XF";
    // console.log(request.headers)
    // console.log(request.body)
    var requiredData = [];
...
router.delete('/admin/researchFund/:researcherId', userController.checkUserInfo, function (request, response) { //ลบ dev. เป็น noti req.
    var methodCode = "XF";
    // console.log(request.headers)
    // console.log(request.body)
    var requiredData = [];

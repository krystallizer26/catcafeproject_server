...
router.get('/reward/', function (request, response) {
    var methodCode = "XF";
    ...
    if (!requiredReady) {
        ...
        flow.exec(
            function () {
                Reward_Control.getRewardByResearcherId(new ObjectId(request.query.researcherId), this);
            },
...
router.get('/admin/reward',async function (request, response) {
    var methodCode = "XF";
    ...
    if (!requiredReady) {
        ...
        if (executable) {
            flow.exec(
                function () {
                    Reward_Control.getRewardByAdmin(researcherId , this);
                },
                function (code, err, getResponse) {
                    if (err) {
                        Return.responseWithCode(ReturnCode.serviceError + methodCode + code, err, response);
                    } else {
                        Return.responseWithCodeAndData(ReturnCode.success, "getRewardByAdmin Successfully executed", getResponse, response);
                    }
                }
            );
        }
...
router.get('/admin/reward/:rewardId',async function (request, response) {
    var methodCode = "XF";

    var requiredData = [];
    requiredData.push(request.params.rewardId);
    requiredData.push(request.headers.token);
    var requiredReady = Validate.requiredData_Check(requiredData)

    console.log("Validation 3")
    var objectIdData = [];
    objectIdData.push(request.params.rewardId);
    var objectIdReady = Validate.objectIDData_Check(objectIdData)
    ...
    if (!requiredReady) {
        ...
        if(user == "token is expired" || user == "user not found"){
            ...
            flow.exec(
                function () {
                    Reward_Control.getRewardForResearcherIdByAdmin(new ObjectId(request.params.rewardId), this);
                },
                function (code, err, getResponse) {
                    if (err) {
                        Return.responseWithCode(ReturnCode.serviceError + methodCode + code, err, response);
                    } else {
                        Return.responseWithCodeAndData(ReturnCode.success, "getRewardForResearcherIdByAdmin Successfully executed", getResponse, response);
                    }
                }
            );
        }
...
router.post('/admin/reward/', userController.checkUserInfo, async function (request, response) { //เพิ่ม dev. เป็น noti req.
    var methodCode = "XF";
...
router.post('/admin/reward/:researcherId', userController.checkUserInfo, async function (request, response) { //แก้ไข dev. เป็น noti req.
    var methodCode = "XF";
    // console.log(request.headers)
    // console.log(request.body)
    var requiredData = [];
...
router.delete('/admin/reward/:researcherId', userController.checkUserInfo, function (request, response) { //ลบ dev. เป็น noti req.
    var methodCode = "XF";
    // console.log(request.headers)
    // console.log(request.body)
    var requiredData = [];
